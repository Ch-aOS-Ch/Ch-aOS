...
This workflow effectively transforms basic asimetric local-first keys into IAM keys. Just think about it, you abstract your private key's content into an versionable provider ID (e.g., bitwarden's IDs), this means you can 1 version control your private keys and 2 _put an RBAC on top of them_, and 3 audit the key's access through the same provider's auditability system which does the transformation's heavy lifting.
