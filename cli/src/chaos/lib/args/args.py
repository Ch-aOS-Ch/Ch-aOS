...
def add_provider_args(parser):
    ...
    provider_group.add_argument(
        ...
        const="default",
        default=None,
        help="Use a configured provider for decryption. If no name is given, uses the default provider.",
    )
...
def add_provider_export_subcommands(subparsers):
    ...
    for provider in providers:
        ...
        providerSub.add_argument(
            "-a",
            "--vault-addr",
            help="Vault address where the token is used (required for vault keys).",
        )
        providerSub.add_argument(
            "-f",
            "--fingerprints",
            nargs="+",
            help="GPG Fingerprint to be exported (required for gpg keys).",
        )
        providerSub.add_argument(
            "-s",
            "--save-to-config",
            action="store_true",
            help="Save the project ID to the chaos config file.",
        )
...
def addSecParsers(secParser):
    ...
    secImport = secSubParser.add_parser(
    ...
    secRotateAdd = secSubParser.add_parser(
        "rotate-add", help="Add new keys to your secrets."
    )
    ...
    secRotateRemove = secSubParser.add_parser(
        "rotate-rm", help="Remove keys from your secrets."
    )
    ...
    secList = secSubParser.add_parser(
        "list", help="Show all keys inside your sops configuration."
    )
    ...
    secList.add_argument(
        ...
        "--value",
        action="store_true",
        help="Only print the key values, useful for piping.",
    )
    ...
    secCat.add_argument(
        ...
        "--value",
        action="store_true",
        help="Only print the value of the key(s), useful for piping.",
    )
    add_provider_args(secCat)

    secShamir = secSubParser.add_parser(
        "set-shamir", help="Manage Shamir's Secret Sharing configuration."
    )
...
def addRambleParsers(rambleParser):
    ...
    rambleEncrypt = rambSubParser.add_parser(
        "encrypt", help="Encrypt a rambling inside a ramble with sops."
    )
    ...
    rambleRead.add_argument(
        ...
        "--values",
        nargs="+",
        help="Only print the value of the rambling(s), useful for piping, only available with --no-pretty.",
    )
    add_provider_args(rambleRead)

    rambleFind = rambSubParser.add_parser(
        "find", help="Find rambles by keyword or tag."
    )
    ...
    rambleMove = rambSubParser.add_parser(
        "move", help="Move a rambling through rambles"
    )
    ...
    rambleDel = rambSubParser.add_parser(
...
def addSetParsers(setParser):
    ...
    secParser = setSubParser.add_parser(
        "secrets", aliases=["sec", "se"], help="Set default secrets file"
    )
    secParser.add_argument("secrets_file", help="Secrets file path")

    sopsParser = setSubParser.add_parser(
        "sops", aliases=["sop"], help="Set default sops file"
    )
...
def addTeamParsers(teamParser):
    ...
    teamClone = teamSubParser.add_parser(
        "clone", help="Clone a team repository locally."
    )
    teamClone.add_argument("target", help="Target team repository in the git format.")

    teamInit = teamSubParser.add_parser(
        "init", help="Initialize a team repository in the current folder."
    )
    teamInit.add_argument(
        "target", help="Target team in the format company.team.person"
    )
    teamInit.add_argument(
        "path",
    ...
    teamActivate.add_argument(
        "path", help="Path where the team repository is located.", nargs="?"
    )

    teamDeactivate = teamSubParser.add_parser("deactivate", help="deactivate a team.")
    teamDeactivate.add_argument(
        "company", help="Company of the team to be deactivated."
    )
    teamDeactivate.add_argument(
        "teams",
        help="Teams to be deactivated, if not passed, will try to remove all teams.",
        nargs="*",
    )
...
def addStyxParsers(styxParser):
    ...
    styxInstall = styxSubParser.add_parser(
        "invoke", help="Install Styx registry entries."
    )
    styxInstall.add_argument(
        "entries", nargs="+", help="Names of the Styx registry entries to install."
    )
    ...
    styxUninstall = styxSubParser.add_parser(
        "destroy", help="Uninstall Styx registry entries."
    )
