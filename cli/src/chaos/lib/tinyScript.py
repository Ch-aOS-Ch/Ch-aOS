...
def runChoboloEdit(chobolo_path):
    ...
    if not chobolo_path:
        ...
        if not os.path.exists(CONFIG_FILE_PATH):
            raise FileNotFoundError(
                "No chaos config file found, and no chobolo path provided."
            )
    ...
    if chobolo_path:
        ...
        try:
            subprocess.run([editor, chobolo_path], check=True)
        except subprocess.CalledProcessError as e:
            raise RuntimeError(
                f"Ch-obolo editing failed. Editor exited with error code {e.returncode}."
            ) from e
